...
int buzzpin = 3;                 // Pulse Sensor purple wire connected to analog pin 0
...
void loop(){
  ...
  int a=analogRead(0);
  sendDataToProcessing('Q', a);
  ledFadeToBeat();
  
  if (stringComplete) {
    receiveDataFromProcessing(inputString); 
    // clear the string:
    inputString = "";
...
void receiveDataFromProcessing(String data){
  if (data.substring(0, 7) == "crystal"){
    int c = data.substring(7,8).toInt();
    if (c > 0) {
      if (data.substring(8,10) == "ON") {
        digitalWrite(c+9, HIGH);
      } else if (data.substring(8,11) == "OFF") {
