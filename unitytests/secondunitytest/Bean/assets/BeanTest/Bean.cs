...
public class Bean : MonoBehaviour {
	...
	private void DidUpdateCharacteristicValueAction(string peripheralId, string service, string characteristic, byte[] data)
	{
		if (peripheralId == deviceSelectedID && characteristic == serialUUID)
		{
			...
			if (asciiString.Substring(0,5) == "DEB0:")
				debug0Text.text = asciiString.Substring(5,asciiString.Length-5); //display test0
			else if (asciiString.Substring(0,5) == "DEB1:")
				debug1Text.text = asciiString.Substring(5,asciiString.Length-5); //display test1 reading in Unity
			else if (asciiString.Substring(0,5) == "DEB2:")
				debug2Text.text = asciiString.Substring(5,asciiString.Length-5); //display test2 reading in Unity
			else if (asciiString.Substring(0,5) == "DEB3:")
				debug3Text.text = asciiString.Substring(5,asciiString.Length-5); //display test3 reading in Unity
			else
